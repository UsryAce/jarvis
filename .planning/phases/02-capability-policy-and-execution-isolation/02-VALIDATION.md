...
Source-grounded classification: `brain.ps1 refresh` validates every generated-path directory component before any deletion or Graphify write, removes any prior intermediate HTML, runs Graphify update, then runs Graphify `cluster-only --no-label` so an incremental no-topology-change result still regenerates JSON/report provenance at the released commit without LLM or provider-token use. It requires `graphify-out/graph.json` plus `graphify-out/GRAPH_REPORT.md` and uses Graphify's tree command to generate a large-graph explorer when the full community visualization is skipped. It repeats the escaped/reparse checks after generation and rejects unsafe destination leaves plus missing, empty, or reparse-point artifacts before copying the complete JSON/report/explorer set, runs `node gsd-tools graphify build snapshot` to rewrite `.planning/graphs/.last-build-snapshot.json`, then syncs vault projections including `explorerAvailable` and `explorerMode`. The three `graphify-out/` paths are ignored/untracked intermediates and are not canonical ownership. After Plan 02-14 freezes approved source, no source may be edited to change this contract.
