...
`powershell -NoProfile -Command "$g = @('graphify-out/graph.json','graphify-out/graph.html','graphify-out/GRAPH_REPORT.md'); foreach ($x in $g) { git check-ignore -q -- $x; if ($LASTEXITCODE) { exit $LASTEXITCODE }; git ls-files --error-unmatch -- $x 2>$null; if (-not $LASTEXITCODE) { exit 1 } }; $started = [DateTime]::UtcNow; powershell -ExecutionPolicy Bypass -File scripts/brain.ps1 refresh; if ($LASTEXITCODE) { exit $LASTEXITCODE }; $graph = Get-Content -Raw -LiteralPath '.planning/graphs/graph.json' | ConvertFrom-Json; $snapshotPath = '.planning/graphs/.last-build-snapshot.json'; $snapshot = Get-Content -Raw -LiteralPath $snapshotPath | ConvertFrom-Json; $vault = if ($env:JARVIS_VAULT_PATH) { $env:JARVIS_VAULT_PATH } else { Join-Path $env:USERPROFILE 'OneDrive/Documents/Codex Agent Brain' }; $vaultGraph = Join-Path $vault 'Projects/Jarvis/Graph'; $statusPath = Join-Path $vaultGraph 'graph-status.json'; $status = Get-Content -Raw -LiteralPath $statusPath | ConvertFrom-Json; $head = (git rev-parse HEAD).Trim(); $explorerPath = (Resolve-Path -LiteralPath '.planning/graphs/graph.html').Path; $fresh = @('.planning/graphs/graph.json','.planning/graphs/GRAPH_REPORT.md','.planning/graphs/graph.html',$snapshotPath,(Join-Path $vaultGraph 'Graphify Report.md'),$statusPath) | ForEach-Object { Get-Item -LiteralPath $_ -ErrorAction Stop }; if ($LASTEXITCODE -or $graph.built_at_commit -ne $head -or $status.sourceCommit -ne $head -or @($graph.nodes).Count -ne @($snapshot.nodes).Count -or @($graph.links).Count -ne @($snapshot.edges).Count -or $status.nodes -ne @($graph.nodes).Count -or $status.edges -ne @($graph.links).Count -or $status.hyperedges -ne @($graph.hyperedges).Count -or -not $status.explorerAvailable -or $status.explorerMode -notin @('full','tree') -or $status.explorer -ne $explorerPath -or @($fresh | Where-Object { $_.LastWriteTimeUtc -lt $started -or $_.Length -le 0 }).Count -or [DateTimeOffset]::Parse([string]$status.generatedAt).UtcDateTime -lt $started) { exit 1 }; if ((Get-FileHash '.planning/graphs/GRAPH_REPORT.md').Hash -ne (Get-FileHash (Join-Path $vaultGraph 'Graphify Report.md')).Hash) { exit 1 }; if (Get-ChildItem -LiteralPath '.planning/graphs',$vaultGraph -Force -Recurse | Where-Object { $_.Name -like '*.brain-txn-*' }) { exit 1 }; powershell -ExecutionPolicy Bypass -File scripts/brain.ps1 status; exit $LASTEXITCODE"`

Source-grounded classification: `brain.ps1 refresh` holds a current-user-protected repository-scoped global mutex from before intermediate mutation through cleanup, validates every generated-path directory component, removes prior intermediate HTML, runs Graphify update and token-free `cluster-only --no-label`, and uses Graphify's tree fallback when full HTML is skipped. It repeats strict repository reparse/containment checks, permits only non-name-surrogate Cloud Files placeholders in the configured OneDrive vault, and generates JSON/report/explorer/snapshot plus vault report/status in isolated stages. Snapshot JSON, topology counts, and exact source provenance are validated before any canonical mutation. Durable unique sibling swaps are then published through `File.Replace`/same-volume `File.Move`, hash-verified, and reverse-rolled back to prior bytes and prior absence on a caught failure; vault status is last. Refresh and sync share this transaction primitive. Successful commit/rollback must leave zero exact transaction residue; incomplete rollback fails closed and retains recovery evidence. This is serialized caught-exception recovery, not a claim of crash/power-loss atomicity across repository and OneDrive or atomic group visibility for readers that ignore the mutex. The three `graphify-out/` paths remain ignored/untracked intermediates, not canonical ownership. After Plan 02-14 freezes approved source, no source may be edited to change this contract.
...
- T-02-GRAPH: Brain refresh leaves stale or mixed canonical/vault generations, loses prior bytes/absence on a late failure, follows a junction/symlink or unknown reparse point, leaves undeclared transaction residue, invokes label-generation tokens, or treats ignored `graphify-out/` intermediates as approved/canonical source.
